...
function createDefaultClubRooms(clubId, createdAt = Date.now()) {
  const insertRoom = db.prepare("INSERT OR IGNORE INTO book_club_rooms (id, club_id, name, slug, icon, room_type, sort_order, created_at) VALUES (?, ?, ?, ?, ?, 'general', ?, ?)");
...
function chapterCount(value) {
  ...
  return Number.isInteger(result) && result >= 1 ? Math.min(result, 200) : 10;
}
function createChapterRooms(clubId, bookId, count, createdAt = Date.now()) {
  const total = chapterCount(count);
  const insertRoom = db.prepare("INSERT OR IGNORE INTO book_club_rooms (id, club_id, book_id, name, slug, icon, room_type, chapter_number, sort_order, created_at) VALUES (?, ?, ?, ?, ?, '#', 'chapter', ?, ?, ?)");
  for (let number = 1; number <= total; number += 1) {
    insertRoom.run(`${clubId}:${bookId}:chapter-${number}`, clubId, bookId, `Chapter ${number}`, `chapter-${bookId}-${number}`, number, 1000 + number, createdAt);
  }
}
function ensureCurrentBookChapterRooms(clubId) {
  const currentBook = db.prepare("SELECT book_id AS bookId, chapter_count AS chapterCount FROM book_club_books WHERE club_id = ? AND is_book_of_month = 1 LIMIT 1").get(clubId);
  if (currentBook) createChapterRooms(clubId, currentBook.bookId, currentBook.chapterCount);
}
function clubRoomForRequest(clubId, roomId) {
  createDefaultClubRooms(clubId);
  ensureCurrentBookChapterRooms(clubId);
  const requested = clean(roomId, 96);
  if (requested) return db.prepare("SELECT id, name, slug, icon, room_type AS roomType, book_id AS bookId, chapter_number AS chapterNumber FROM book_club_rooms WHERE club_id = ? AND (id = ? OR slug = ?)").get(clubId, requested, requested) || null;
  return db.prepare("SELECT id, name, slug, icon, room_type AS roomType, book_id AS bookId, chapter_number AS chapterNumber FROM book_club_rooms WHERE club_id = ? AND slug = 'lobby'").get(clubId);
}
...
app.get("/api/book-clubs", authRequired, (req, res) => {
  ...
  const roomsForClub = db.prepare("SELECT r.id, r.name, r.slug, r.icon, r.room_type AS roomType, r.book_id AS bookId, r.chapter_number AS chapterNumber, (SELECT COUNT(*) FROM book_club_messages m WHERE m.room_id = r.id) + CASE WHEN r.room_type = 'chapter' THEN (SELECT COUNT(*) FROM book_club_discussions d WHERE d.club_id = r.club_id AND d.text LIKE '[Chapter ' || r.chapter_number || '%] %') ELSE 0 END AS messageCount FROM book_club_rooms r WHERE r.club_id = ? ORDER BY r.sort_order, r.name");
  const currentBookForClub = db.prepare("SELECT book_id AS bookId, chapter_count AS chapterCount FROM book_club_books WHERE club_id = ? AND is_book_of_month = 1 LIMIT 1");
  return res.json({ clubs: clubs.map((club) => { createDefaultClubRooms(club.id, club.createdAt); const currentBook = currentBookForClub.get(club.id); if (currentBook) createChapterRooms(club.id, currentBook.bookId, currentBook.chapterCount, club.createdAt); return { ...club, members: membersForClub.all(club.id), rooms: roomsForClub.all(club.id) }; }) });
});
...
app.put("/api/book-clubs/:id/books/:bookId/book-of-month", authRequired, writeLimit, (req, res) => { const clubId = clean(req.params.id, 64); if (!isClubOwner(clubId, req.authUser.id)) return res.status(403).json({ message: "Only the club owner can choose the current book." }); const bookId = clean(req.params.bookId, 64); if (!db.prepare("SELECT 1 FROM books WHERE id = ? AND user_id = ? AND is_owned = 1").get(bookId, req.authUser.id)) return res.status(404).json({ message: "Choose one of your shelf books as the current read." }); const existing = db.prepare("SELECT 1 FROM book_club_books WHERE club_id = ? AND book_id = ?").get(clubId, bookId); const count = chapterCount(req.body.chapterCount); if (!existing) db.prepare("INSERT INTO book_club_books (club_id, book_id, added_by, chapter_count, added_at) VALUES (?, ?, ?, ?, ?)").run(clubId, bookId, req.authUser.id, count, Date.now()); db.prepare("UPDATE book_club_books SET is_book_of_month = 0 WHERE club_id = ?").run(clubId); db.prepare("UPDATE book_club_books SET is_book_of_month = 1, chapter_count = ? WHERE club_id = ? AND book_id = ?").run(count, clubId, bookId); createChapterRooms(clubId, bookId, count); return res.json({ message: "Current book updated.", chapterCount: count }); });
