...
app.get("/api/book-clubs", authRequired, (req, res) => {
  ...
  const roomsForClub = db.prepare("SELECT r.id, r.name, r.slug, r.icon, r.room_type AS roomType, r.book_id AS bookId, r.chapter_number AS chapterNumber, COUNT(m.id) AS messageCount FROM book_club_rooms r LEFT JOIN book_club_messages m ON m.room_id = r.id WHERE r.club_id = ? GROUP BY r.id ORDER BY r.sort_order, r.name");
