...
test("book club members can use group chat while outsiders are denied", async () => {
  ...
  const clubList = await request(app).get("/api/book-clubs").set("Authorization", `Bearer ${owner.token}`).expect(200);
  const lobby = clubList.body.clubs[0].rooms.find((room) => room.slug === "lobby");
  const intros = clubList.body.clubs[0].rooms.find((room) => room.slug === "intros");
  assert.ok(lobby);
  assert.ok(intros);
  await request(app).post(`/api/book-clubs/${club.body.club.id}/messages`).set("Authorization", `Bearer ${member.token}`).send({ roomId: lobby.id, text: "Ready for chapter one?" }).expect(201);
  await request(app).post(`/api/book-clubs/${club.body.club.id}/messages`).set("Authorization", `Bearer ${member.token}`).send({ roomId: intros.id, text: "Hi, I'm new here." }).expect(201);
  const messages = await request(app).get(`/api/book-clubs/${club.body.club.id}/messages?roomId=${encodeURIComponent(lobby.id)}`).set("Authorization", `Bearer ${owner.token}`).expect(200);
  assert.equal(messages.body.room.slug, "lobby");
  assert.equal(messages.body.messages.length, 1);
  assert.equal(messages.body.messages[0].fromEmail, member.user.email);
  assert.equal(messages.body.messages[0].roomId, lobby.id);
